...
/** Boni: aktivierte Demo-Promotions und aktueller Bonus-/Freirundenstand. Rein additiv, ohne Bedingungen. */
  ...
  return (
    <div className="anim-panel-in space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Card>
          <p className="text-sm text-muted">Bonus-Credits (Demo)</p>
      ...
      <section aria-labelledby="grants-title" className="space-y-3">
        <h2 id="grants-title" className="text-md font-semibold text-primary">
          Aktivierte Demo-Promotions
        </h2>
        {!hydrated ? (
          <Skeleton className="h-24 w-full" />
        ) : grants.length === 0 ? (
          <EmptyState icon={<Gift aria-hidden="true" />} title="Noch keine Promotion aktiviert." text="Auf der Promotions-Seite kannst du Beispiele als Demo aktivieren." action={<LinkButton href="/promotions" variant="primary">Zu den Promotions</LinkButton>} compact />
        ) : (
            ...
            {grants.map((g) => {
              ...
              return (
                <li key={g.id} className="flex items-center justify-between gap-3 p-4 text-sm">
                  <div>
                    <p className="font-medium text-primary">{promo?.title ?? "Demo-Bonus"}</p>
