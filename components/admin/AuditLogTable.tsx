            ...
            <table className="w-full min-w-[820px] text-sm">
              ...
              <thead className="bg-elevated text-left text-xs uppercase tracking-wider text-muted">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3 font-medium">Von</th>
                  <th scope="col" className="px-4 py-3 font-medium">Aktion</th>
                  <th scope="col" className="px-4 py-3 font-medium">Entität</th>
                  <th scope="col" className="px-4 py-3 font-medium">Vorher → Nachher</th>
                </tr>
              </thead>
              <tbody>
                {entries.map((entry) => (
                  <tr key={entry.id} className="border-t border-border-subtle align-top">
                    <td className="tabular whitespace-nowrap px-4 py-3 text-muted">{formatDateTime(entry.createdAt)}</td>
                    <td className="px-4 py-3 text-primary">{entry.actorUserId}</td>
                    <td className="px-4 py-3 text-primary">{entry.action}</td>
                    <td className="px-4 py-3 text-primary">
                      {entry.entityType} · {entry.entityId}
                    </td>
                    <td className="max-w-xs break-all px-4 py-3 text-xs text-muted">
                      {summarizeChange(entry.before)} → {summarizeChange(entry.after)}
                    </td>
                  </tr>
                ))}
