      ...
      <table className="w-full min-w-[440px] text-sm">
        ...
        <tbody>
          {paytable.entries.map((e) => (
            <tr key={e.key} className="border-t border-border-subtle even:bg-elevated/40">
              <td className="px-4 py-2.5 text-primary">{e.label}</td>
              <td className="tabular px-4 py-2.5 text-right text-primary">{formatMultiplier(e.multiplier)}</td>
              <td className="tabular px-4 py-2.5 text-right text-primary">{formatPercent(e.probability, e.probability < 0.001 ? 4 : 2)}</td>
              <td className="tabular px-4 py-2.5 text-right text-muted">{(e.multiplier * e.probability).toFixed(4).replace(".", ",")}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr className="border-t border-gold/50 bg-elevated font-medium">
            ...
            <td className="tabular px-4 py-3 text-right">{formatPercent(sum, 2)}</td>
            <td className="tabular px-4 py-3 text-right text-gold">{ev.toFixed(4).replace(".", ",")}</td>
          </tr>
        </tfoot>
      </table>
    </div>
